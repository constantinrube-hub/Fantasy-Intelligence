...
const ReplacementService={
  cutoff(position,{league=window.state?.league,players=(typeof PLAYERS!=='undefined'?PLAYERS:window.PLAYERS||[]),state=window.state,valueFn=p=>finite(p.engineSeasonProjection)??finite(p.sleeperSeasonProjection)??finite(p.modelScore)??0}={}){const pos=canonicalPos(position),saved=finite(state?.projectedReplacementLevels?.[pos]?.cutoff);if(saved&&saved>=1)return Math.round(saved);const demand=LeagueDemandService.starterDemand({league,players,valueFn}).leagueStarterDemand[pos]||1;const benchSlots=(league?.roster_positions||[]).filter(s=>!PositionRegistry.isStarter(s)).length,starterSlots=PositionRegistry.starterSlots(league?.roster_positions||[]).length,benchInfluence=Math.max(0,finite(state?.replacement?.benchInfluence)??0)/100,benchExtra=starterSlots?Math.round(demand*(benchSlots/starterSlots)*benchInfluence):0;return Math.max(1,Math.round(demand+benchExtra));},
...
window.FIECore={version:'9.1-consolidation',PositionRegistry,PlayerIdentity:{id:playerId,canonicalPosition:canonicalPos},LineupOptimizer,LeagueDemandService,ReplacementService,RosterValueService,ContextFingerprint,Diagnostics};
})();
