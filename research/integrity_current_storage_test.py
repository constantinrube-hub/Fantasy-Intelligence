...
for p in paths:
 ...
 gov=read_json(p.parents[1]/'governance/active_release.json',{}) or {};assert (gov.get('checks') or {}).get('current_storage_integrity') is True,f'governance did not validate shared current storage: {lid}'
 line=gov.get('model_lineage') or {};assert (line.get('artifact_sha256') or {}).get('current_snapshot')==sha(p),f'governance current manifest hash mismatch: {lid}'
 governed=line.get('shared_current_artifacts') or {}
 for key in ('player_base','scoring_overlay'):
  row=governed.get(key) or {};assert row.get('path')==st[key],f'governance shared path mismatch: {lid} {key}'
...
# Multiple overlays for one scoring signature are allowed after partial refreshes
# when time-varying projections conflict. Exact duplicates still collapse because
