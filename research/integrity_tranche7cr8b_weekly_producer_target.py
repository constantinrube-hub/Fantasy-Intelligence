...
def main(argv: list[str] | None = None) -> int:
    ...
    assert target["artifact_policy"] == "controlled_no_network_fixture_only"
    assert preflight["lifecycle"] == "closed_manual_validation"
    if args.mode == "target":
        assert target["lifecycle"] == "target_active"
        ...
        assert WORKFLOW not in lifecycle["active_controlled_workflows"]
        assert not re.search(r"(?m)^  push:", workflow)
        assert target["validated_target"]["commit"] == "9e1ce95"
    ...
    print(f"PASS R8B {args.mode}: controlled fixture proves frozen weekly capture without network, schedule, or app activation")
    return 0
