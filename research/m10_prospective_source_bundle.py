...
def validate_input(path: Path) -> tuple[dict[str, Any], dict[str, Path]]:
    ...
    assert value.get("schema") == INPUT_SCHEMA and value.get("research_only") is True
...
def create_bundle(input_manifest: Path, output_root: Path) -> dict[str, Any]:
    ...
    records = [{key: row[key] for key in ("role", "path", "sha256", "captured_at", "as_of", "point_in_time_eligible", "historical_reconstruction")} for row in value["source_records"]]
