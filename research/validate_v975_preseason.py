...
def main():
    ...
    if missing:
        raise AssertionError(f"missing V9.7.5 outputs: {missing}")
    ...
    assert r.get("status") in {
        "complete_research_only","blocked_insufficient_chronological_folds"
    }
    ...
    # V9.7.2 must still be exact for this V9.7.5 research artifact to exist.
    assert src.get("all_v972_folds_exact_scoring_replay") is True

    # Exact historical M9 is required for promotion/readiness, but not for a
    # diagnostic-only V9.7.5 artifact. preseason_projection_v5.py already uses
    # exact_m9 inside football_ready, so the correct validator contract is:
    #   exact M9 false => valid artifact, but promotion/readiness MUST stay false.
    m9_exact=src.get("all_m9_folds_exact_scoring_replay") is True
    qb=(r.get("per_position") or {}).get("QB") or {}
    review=list(r.get("football_model_promotion_review_positions") or [])
    expected=list(r.get("expected_season_points_ready_positions") or [])

    if not m9_exact:
        assert qb.get("football_model_promotion_review_ready") is False
        assert qb.get("expected_season_points_ready") is False
        assert qb.get("production_activation_allowed") is False
        assert not review
        assert not expected
    ...
    if qb.get("football_model_promotion_review_ready"):
        ...
        for k in [
            "ppg_mae_better_or_equal_best_standalone",
            "full_schedule_mae_better_or_equal_best_standalone",
    ...
    if qb.get("expected_season_points_ready"):
        ...
        assert (qb.get("standalone_noninferiority") or {}).get(
            "expected_season_mae_better_or_equal_best_standalone"
        ) is True

    params=pd.read_csv(root/"preseason_v975_params.csv",low_memory=False)
    if not params.empty:
        assert {
            "test_season","weight_v972","weight_m9","calibration_enabled",
            "weight_training_seasons"
        }.issubset(params.columns)
        weights=pd.to_numeric(params.weight_v972,errors="coerce")
        assert ((weights>=0)&(weights<=1)).all()
        for _,row in params.iterrows():
            train=[
                int(x) for x in str(row.weight_training_seasons).split(",")
                if x and x!="nan"
            ]
            assert all(x < int(row.test_season) for x in train), (
                row.test_season,train
            )

    pred=pd.read_csv(root/"preseason_v975_predictions.csv",low_memory=False)
    if not pred.empty:
        need={
            "position","test_season","canonical_player_id","actual_ppg",
            "v972_pred_ppg","m9_pred_ppg","ensemble_pred_ppg",
            "actual_season_points","ensemble_pred_season_points","weight_v972"
        }
