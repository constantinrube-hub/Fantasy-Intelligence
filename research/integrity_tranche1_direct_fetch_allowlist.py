...
ALLOW={
    ...
    "app/v9.3.3-runtime-integrity.js":"RAW_FETCH_FALLBACK",
...
def main():
    ...
    for p in sorted((ROOT/"app").rglob("*.js")):
        ...
        if has_direct(txt):
            observed[rel]=ALLOW.get(rel,"UNALLOWLISTED")
    ...
    # Every allowlisted fallback must still visibly delegate/prefer the
    # canonical owner.  A fallback is not permission to become a new primary
    # transport path silently.
    for p,token in FALLBACK_TOKEN.items():
        if p in observed:
            txt=(ROOT/p).read_text(encoding="utf-8")
            assert token in txt, (
                f"{p} raw fetch exists without expected canonical/fallback "
                f"owner token {token}"
            )

    rr="app/core/research-report-service.js"
    if a.mode=="baseline":
        assert observed.get(rr)=="KNOWN_PRIMARY_RAW_FETCH", (
            "baseline must reproduce ResearchReportService primary raw fetch"
        )
        print(
            "KNOWN_GAP_REPRODUCED ResearchReportService primary raw fetch; "
            "all other direct app fetches are allowlisted transport/fallback paths"
        )
    else:
        assert rr not in observed, (
            "target: ResearchReportService must route primary transport "
            "through FIEDataClient"
        )
