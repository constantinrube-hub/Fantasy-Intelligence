...
fixture={'league_id':'1316165875291668480','name':'Fixture','season':'2026','season_type':'regular','total_rosters':12,
         'roster_positions':['QB','RB','WR','TE','FLEX','BN'],'settings':{'type':2},'scoring_settings':{'rec':1,'pass_td':4}}
...
for token in ['nfl_entry_cohort_floor','cohort_floor_with_legacy_cap','Legacy veteran cap','managedLeagues','isPlayerEligible']:
    assert token in js
html=(ROOT/'index.html').read_text(encoding='utf-8')
assert 'app/portfolio-config.js' in html
assert 'FIEPortfolioConfig?.isPlayerEligible' in html
assert 'FIEPortfolioConfig?.acquisitionConstraint' in html
print('PASS: 19-league portfolio + fixed-cohort custom rules are valid and research-fingerprinted')
