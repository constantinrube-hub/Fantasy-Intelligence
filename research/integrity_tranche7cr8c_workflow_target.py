...
def main(argv: list[str] | None = None) -> int:
    ...
    assert target["artifact_policy"] == "controlled_no_network_operational_dry_run"
    ...
    if args.mode == "target":
        ...
        assert target["validated_target"] == {"commit": "9d9e613", "status": "DEPLOYABLE_SOURCE"}
    ...
    print(f"PASS R8C {args.mode}: controlled operational dry run is no-network, read-only, and non-production")
    return 0
