...
def audit_league(lid: str, row: dict, issues: list[str], warnings: list[str]) -> dict:
    ...
    gd = load(gov, {}) or {} if gov.exists() else {}
    return {
        "league_id": lid,
        "format": profile.get("format") or row.get("format"),
        "artifact_ready": not missing and bool(fp) and bool(sig),
        "current": current.exists(),
        "governance": gov.exists(),
        "runtime_research_active": bool(gd.get("runtime_enabled") is True and gd.get("runtime_allow_m5") is True),
        "runtime_reason": gd.get("reason") if gd else None,
    }
...
def main():
    ...
    artifact_ready = bool(rows) and all(r.get("artifact_ready") for r in rows) and not issues
    active_count = sum(1 for r in rows if r.get("runtime_research_active"))
    runtime_status = "RUNTIME_RESEARCH_ACTIVE" if active_count else "RUNTIME_FALLBACK_ONLY"
    result = {
        "status": "FAIL" if issues else ("WARN" if warnings else "REPOSITORY_READY"),
        "repository_status": "RESEARCH_ARTIFACT_READY" if artifact_ready else "RESEARCH_ARTIFACT_NOT_READY",
        "runtime_status": runtime_status,
        "runtime_research_active_leagues": active_count,
