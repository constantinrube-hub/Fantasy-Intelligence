...
function playerId(p){return String(p?.sleeperId||p?.name||'');}
...
function simulationContext(rosterId){
  const h=draftHelpers(),d=currentDraft();if(!d||!h.buildDraftValueRows)return null;
  const rows=h.buildDraftValueRows(rosterId)||[],pool=draftCandidatePool(),startPick=currentPick();
...
function draftPanel(){
  ...
  if(!cached){
    panel.innerHTML=`<div class="eyebrow">Remaining-draft simulation</div><h3 style="margin:4px 0 6px">Monte Carlo Draft Strategist <span class="badge">Beta</span></h3><div class="subtitle">Evaluates a pick by the final roster it tends to produce, not only by the player's standalone rank. Simulations use Sleeper ADP uncertainty, roster needs, saved manager/position tendencies when available, and the active league-format utility.</div>${onClock?`<button id="runFieDraftMonteCarlo" class="btn primary" style="margin-top:10px">Simulate top choices</button>`:`<div class="notice" style="margin-top:10px">Your selected roster is not on the clock. Its next pick is #${next?.pickNo??'—'}. Choice simulation activates when that roster is on the clock.</div>`}`;
  }else{
  ...
  const btn=document.getElementById('runFieDraftMonteCarlo');if(btn&&!btn.dataset.bound){btn.dataset.bound='1';btn.onclick=()=>{btn.disabled=true;btn.textContent='Simulating…';setTimeout(()=>{runDraftMonteCarlo(rosterId);draftPanel();},0);};}
}
...
function augmentDrawer(id){
  const body=document.getElementById('drawerBody'),p=(window.PLAYERS||PLAYERS||[]).find(x=>String(x.sleeperId||x.name)===String(id));if(!body||!p||body.querySelector('.fie-decision-summary'))return;const x=playerDecisionSummary(p),d=document.createElement('div');d.className='fie-decision-summary';d.innerHTML=`<div class="eyebrow">Decision summary</div><div class="fie-action-grid"><div class="fie-action-card"><span>Week ${activeWeek()}</span><b>${fmtV(x.weekly)}</b><small>${x.weeklyDelta===null?'No Sleeper baseline':`${x.weeklyDelta>=0?'+':''}${fmtV(x.weeklyDelta,1)} vs Sleeper`}</small></div><div class="fie-action-card"><span>Season</span><b>${fmtV(x.season)}</b><small>${x.seasonDelta===null?'No baseline':`${x.seasonDelta>=0?'+':''}${fmtV(x.seasonDelta,1)} vs Sleeper`}</small></div><div class="fie-action-card"><span>League VOR</span><b>${x.vor===null?'—':`${x.vor>=0?'+':''}${fmtV(x.vor,1)}`}</b><small>league-specific scarcity</small></div><div class="fie-action-card"><span>Market edge</span><b>${x.edge===null?'—':`${x.edge>=0?'+':''}${fmtV(x.edge,1)}`}</b><small>ranking vs ADP</small></div></div><div class="fie-explain"><b>Why / counterfactual:</b> ${x.factors.length?x.factors.map(f=>`${escV(f.k)} ${f.v>=0?'+':''}${fmtV(f.v,1)}%`).join(' · '):'No active model overlays available.'}${x.swap?`<br><b>If added to your roster:</b> best modeled drop is ${escV(x.swap.drop.name)}, roster utility ${x.swap.gain>=0?'+':''}${fmtV(x.swap.gain,2)}.`:''}${x.dv!==null?`<br><b>Format-specific decision value:</b> ${fmtV(x.dv,2)}.`:''}<br><span class="muted">This separates the market baseline, football/model overlay, league scarcity and roster counterfactual instead of hiding them inside one score.</span></div>`;body.insertAdjacentElement('afterbegin',d);
}
...
function renderMatchupPanel(){
  const p=ensureMatchupPanel(),active=state.activeTab==='matchupsim';p.classList.toggle('active',active);if(!active)return;document.getElementById('mainArea')?.classList.add('hidden');document.getElementById('weeklyControls')?.classList.add('hidden');
...
function wrapDraftRenderer(){
  ...
  function wrapped(){const r=prev.apply(this,arguments);try{draftPanel();}catch(e){console.warn('FIE draft simulation panel',e);}return r;}
