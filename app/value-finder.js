...
function vfPositions(){const slots=window.FIERuntimeContracts?.roster_slots||{},seen=new Set();for(const slot of state.league?.roster_positions||[])for(const p of (slots[String(slot).toUpperCase()]?.eligible||[]))seen.add(p);return [...seen].filter(p=>!['OL'].includes(p));}
const VF_CACHE={baseKey:null,baseRows:null,liveKey:null,liveMap:null,builds:0,liveBuilds:0};
...
function vfInvalidate(reason='manual'){VF_CACHE.baseKey=null;VF_CACHE.baseRows=null;VF_CACHE.liveKey=null;VF_CACHE.liveMap=null;VF_CACHE.lastInvalidation=reason;}
