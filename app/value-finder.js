/* Fantasy Intelligence Engine · Draft Value Finder · V8.9-VF1
...
function vfPolicyScore(p,ctx,profile){
  ...
  const vals={
    ...
    current_role:vfNum(p.currentOpportunity,50),
...
function vfLiveDraftMap(){
  try{
    ...
    const rows=window.FIE_DRAFT_V71?.buildDraftValueRows?.(rid)||[];
    return new Map(rows.map(x=>[String(x.p?.sleeperId),x]));
...
function vfTop100RowHTML(x,i){
  const edge=x.overallEdge===null?'—':`${x.overallEdge>=0?'+':''}${x.overallEdge}`,cap=x.valueCapture===null?'—':`${x.valueCapture}%`,pathCls=x.pathDelta>=0?'vf-path-positive':'vf-path-negative',types=(x.opportunityTypes||[]).map(t=>`<span class="vf-type ${t==='WAIT VALUE'?'wait':t==='MARKET VALUE'?'value':''}">${vfEsc(t)}</span>`).join('');
  return `<tr data-vf-id="${vfEsc(x.p.sleeperId||x.p.name)}"><td><b>${i+1}</b></td><td><div class="vf-player">${vfEsc(x.p.name)}</div><div class="muted">${vfEsc(x.p.position)} · ${vfEsc(x.p.team)}</div></td><td><b>${vfFmt(x.adp,0)}</b><br><span class="muted">market #${x.marketOverallRank??'—'}</span></td><td><b>#${x.fieLeagueRank??'—'}</b><br><span class="muted">comparable #${x.fieOverallRank??'—'} · edge ${edge}</span></td><td><b>${vfFmt(x.policy.score)}</b><br><span class="muted">roster ${vfFmt(x.rosterFit,0)}</span></td><td class="${vfRiskClass(x.tierRisk)}">${x.tierRisk}<br><span class="muted">drop ${vfFmt(x.replacementDrop)}${x.replacement?` → ${vfEsc(x.replacement.p.name)}`:''}</span></td><td><b>${x.survive??'—'}${x.survive!==null?'%':''}</b><br><span class="muted">pressure ${x.pressureN?`${x.opponentPressure>=0?'+':''}${vfFmt(x.opponentPressure)}`:'none'}</span></td><td><b>${cap}</b><br><span class="muted">reach cost ${vfFmt(x.reachCost)} picks</span></td><td><b>${vfFmt(x.waitCost)}</b><br><span class="muted">expected policy pts</span></td><td class="${pathCls}">${x.pathDelta>=0?'+':''}${vfFmt(x.pathDelta)}<br><span class="muted">3-pick proxy</span></td><td><b>${vfFmt(x.evidenceScore,0)}</b><br><span class="muted">${x.histValidated?'validated':'diagnostic'} ${vfM5Position(x.p)}</span></td><td>${types||'<span class="muted">—</span>'}</td><td><span class="vf-state ${vfStateClass(x.optimizerAction)}">${vfEsc(x.optimizerAction)}</span></td></tr>`;
}
function renderTop100Optimizer(){
  ...
  const rowHtml=shown.map(vfTop100RowHTML).join('')||'<tr><td colspan="13"><div class="empty">No Top-100 candidates meet the current filters.</div></td></tr>';
  box.innerHTML=`${vfControlsHTML()}${vfTop100ControlsHTML(ctx)}<div class="vf-opt-grid"><div class="vf-opt-card"><span class="filter-label">TAKE NOW</span><div class="big">${take}</div><div class="tiny">waiting has meaningful EV cost</div></div><div class="vf-opt-card"><span class="filter-label">WAIT</span><div class="big">${wait}</div><div class="tiny">good player, price can improve</div></div><div class="vf-opt-card"><span class="filter-label">Tier cliffs</span><div class="big">${cliffs}</div><div class="tiny">Tier Drop Risk 65+</div></div><div class="vf-opt-card"><span class="filter-label">Avg value captured</span><div class="big">${avgCap}%</div><div class="tiny">of available FIE→market discount</div></div><div class="vf-opt-card"><span class="filter-label">Path horizon</span><div class="big">3 picks</div><div class="tiny">take-now vs wait proxy</div></div></div><div class="scroll" style="max-height:62vh"><table class="vf-table"><thead><tr><th>#</th><th data-vf100-sort="player">Player</th><th data-vf100-sort="adp">ADP</th><th data-vf100-sort="edge">FIE overall</th><th>Policy / roster</th><th data-vf100-sort="tier">Tier drop</th><th data-vf100-sort="survive">Survive</th><th data-vf100-sort="capture">Value capture</th><th>Wait cost</th><th data-vf100-sort="path">3-pick path Δ</th><th>Evidence</th><th>Value type</th><th data-vf100-sort="optimizer">Action</th></tr></thead><tbody>${rowHtml}</tbody></table></div><div class="notice" style="margin-top:10px"><b>Top-100 logic:</b> FIE rank and Sleeper market rank use the exact league-eligible player sample. <b>Value Capture</b> estimates how much of the FIE→market discount you have already captured at the planned pick. <b>Reach Cost</b> penalizes taking a likely survivor too early. <b>Tier Drop</b> estimates the same-position quality lost if you wait. <b>3-pick path Δ</b> is a planning proxy comparing “take this player now” with “take the best alternative now and try to recover this player later”; it is not a claim of a fully solved stochastic draft.</div>`;
...
function vfFilteredRows(){
  const f=state.valueFinder,live=vfLiveDraftMap();
  let rows=vfBuildRows().filter(x=>vfBandMatch(x.adp,f.band));
...
<label><span class="filter-label">Position</span><select id="vfPosition"><option value="ALL">All positions</option>${POSITIONS.map(p=>`<option value="${p}" ${f.position===p?'selected':''}>${p}</option>`).join('')}</select></label>
...
function vfRowHTML(x,i){
  const fieRank=x.fiePosRank?`${x.p.position}${x.fiePosRank}`:'—';
  const edge=x.posEdge===null?'—':`${x.posEdge>=0?'+':''}${x.posEdge} spots`;
  const survive=x.live?.survive!==null&&x.live?.survive!==undefined?`<div class="vf-draft-mini">${x.live.survive}% survives next pick</div>`:'';
  return `<tr data-vf-id="${vfEsc(x.p.sleeperId||x.p.name)}"><td><b>${i+1}</b></td><td><div class="vf-player">${vfEsc(x.p.name)}</div><div class="muted">${vfEsc(x.p.position)} · ${vfEsc(x.p.team)} · ${vfEsc(vfExperience(x.p))}</div></td><td><b>${vfFmt(x.adp,0)}</b><br><span class="muted">eligible ${x.p.position}${x.marketPosRank??'—'}</span></td><td class="${(x.posEdge||0)>=4?'vf-rankedge':''}"><b>${fieRank}</b><br><span class="muted">${edge}</span></td><td><b>${vfFmt(x.policy.score)}</b><br><span class="muted">${Math.round(x.policy.coverage*100)}% inputs</span></td><td><b>${x.snap.score}</b><br><span class="muted">${vfEsc(x.snap.label)}</span></td><td>${vfEvidenceChips(x)}</td><td><b>${vfFmt(x.strength)}</b><br><span class="muted">${vfEsc(x.confidence)}</span></td><td><b>${vfEsc(x.window)}</b><br><span class="muted">market ${vfFmt(x.adp,0)}</span></td><td><span class="vf-state ${vfStateClass(x.state)}">${vfEsc(x.state)}</span>${survive}</td><td class="vf-why vf-hide-mobile">${vfEsc(vfWhy(x))}<div class="vf-source">${vfEsc(x.p.path||'')} · ${vfEsc(x.p.opportunitySource||'')}</div></td></tr>`;
}
...
function renderValueFinder(){
  ...
  const research=vfM5Research();if(!research&&window.FIE_M5?.loadMilestone5Research){status.textContent='Loading M5 draft evidence…';window.FIE_M5.loadMilestone5Research().then(()=>renderValueFinder()).catch(()=>renderValueFinder());}
  if(state.valueFinder.band==='LT100'){renderTop100Optimizer();return;}
  const all=vfBuildRows(),rows=vfFilteredRows(),shown=rows.slice(0,Number(state.valueFinder.limit)||10),band=BAND_LABELS[state.valueFinder.band]||state.valueFinder.band;
  ...
  status.innerHTML=`<b>${vfEsc(state.league.name||state.league.league_id)}</b> · ${vfEsc(band)} · ${rows.length} qualifying target${rows.length===1?'':'s'} · market ranks are recalculated inside this league's legal player pool. ${research?'M5 historical evidence loaded.':'M5 historical evidence pending; fallback scoring shown.'}`;
  const rowHtml=shown.map(vfRowHTML).join('')||'<tr><td colspan="11"><div class="empty">No players meet the current Value Finder filters. Broaden the ADP band, snap path or confidence filter.</div></td></tr>';
  ...
  <div class="scroll" style="max-height:62vh"><table class="vf-table"><thead><tr><th>#</th><th data-vf-sort="player">Player</th><th data-vf-sort="adp">Sleeper ADP</th><th data-vf-sort="edge">FIE vs Pos ADP</th><th data-vf-sort="policy">Policy</th><th data-vf-sort="snap">Snap path</th><th>Evidence</th><th data-vf-sort="strength">Target strength</th><th>Target window</th><th>Live plan</th><th class="vf-hide-mobile">Why</th></tr></thead><tbody>${rowHtml}</tbody></table></div>
  <div class="notice" style="margin-top:10px"><b>How to read this:</b> Value Finder is a discovery layer, not a replacement for the Draft Assistant. <b>100–200</b> emphasizes league/model mispricing; <b>200+</b> weights snap-path certainty most heavily. Positional ADP is always rebuilt inside the league-eligible universe, so excluded Genesis players never create artificial gaps. M5 historical evidence changes confidence, while M6 remains governed normally and is never force-enabled here.</div>`;
...
function augmentDraftAssistant(){
  ...
  for(const tr of table.querySelectorAll('tbody tr')){
    const name=(tr.children[1]?.querySelector('b')?.textContent||'').trim(),p=PLAYERS.find(x=>x.name===name);if(!p)continue;
