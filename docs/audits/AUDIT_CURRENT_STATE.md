...
- Current authorized boundary: Tranche 7C-R8 may implement the separately designed, default-branch operational workflow plus its audit-branch controlled target. It must load the frozen R7 lock, remain research-only with M9 as champion, and prove time-window/path allowlists before any merge to `main`; it cannot alter app/runtime/rankings, create 6F, or activate a model.
