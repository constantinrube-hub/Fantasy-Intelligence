...
- Current authorized boundary: Tranche 7C-R7 real 2026 season-lock target. Its first run `33986847195` produced a verified 13,379,405-byte historical input artifact but failed because a blanket non-negative label assertion rejected legitimate negative rushing yards. Sol resolved the target domain in `TRANCHE7CR7_TARGET_DOMAIN_DECISION.md`: negative continuous yardage is preserved, count targets remain non-negative, null labels remain null, and the zero floor is post-inference only. Terra may implement and rerun this bounded target. M9 remains champion; no scheduler, app/runtime/ranking change, 6F path, or default-branch activation is authorized.
