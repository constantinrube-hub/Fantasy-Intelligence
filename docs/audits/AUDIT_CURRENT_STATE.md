...
- Current authorized boundary: Terra High may implement Tranche 7C-R8A corrected-lock preflight only: shared time-safe feature construction, a v2 lock with out-of-sample residual-vector calibration and complete provenance, activation-guard v1 rejection, and focused tests. It must stop after the real v2 lock target for external artifact verification. R8B weekly producer and R8C default-branch workflow are ordered follow-ons; M9 remains champion and app/runtime/rankings, 6F, shadow use, and production activation remain forbidden.
