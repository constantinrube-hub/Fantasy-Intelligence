...
- Last validated target head: `c00095fe1df6fbec8686f869ad833071195350e5` — Tranche 7C-R5 activation-guard target green, GitHub Actions run `33986191685`, success in 30 seconds; it proves that prospective writes require a real non-fixture 2026 lock, `main`, and the research-only path allowlist
- Last validated target head: `1b77775dfb89b478f440645c88ee2905fe9f1567` — Tranche 7C-R7 real 2026 season-lock target green, GitHub Actions run `33988276897`, success in 1m 48s, `DEPLOYABLE_SOURCE`; verified artifact SHA-256 `dfad49d5953eaec41f8886ee34a4066f4711673d4ff701597e080d7012079a6b` and immutable lock SHA-256 `f3bd91a5002f6052d5d2d27e55b0747e0b1cb141ca624a534608ae9559e23eb8`
...
- General Season Preview: `GENERAL_SEASON_PREVIEW_MODEL_DESIGN.md`, `config/general-season-preview-model-design.json`, `research/general_season_preview.py`, and `research/integrity_general_season_preview_test.py` implement a bounded research-only Phase A raw-stat forecast followed by Phase B exact league scoring. The initial real build on `main` at `08a78e887b1198089cc4b3d0e93dfa6ede34e32f` (GitHub Actions run `34277179918`) produced the preserved `general-preview-v1` / `league-preview-v2` bundle, but a later crosscheck invalidated it for downstream use: `LAR` player rows did not bind to the `LA` team budget, QB completions were not reconciled to team completions, and player receiving yards/TDs were not reconciled to passing budgets. The corrected immutable namespace is `general-preview-v2` / `league-preview-v3`; it adds p50 and all-scenario accounting audit gates and requires a new green build before Phase C or reports consume it. Existing `season-preview-v1` remains immutable; M9, M10, app/runtime, canonical rankings, ADP separation, and promotion authority remain unchanged.
- Season Preview Phase C design: `SEASON_PREVIEW_PHASE_C_DRAFT_VALUE_DESIGN.md` and `config/season-preview-phase-c-draft-value-design.json` authorize a bounded Terra implementation of canonical Core replacement/VOR, current-season value ranks, and frozen pre-kickoff ADP comparison over the completed Phase A/B artifacts. The design explicitly blocks pseudo-complete dynasty, chopped, and best-ball ranks where long-horizon or weekly-shape evidence is absent. It authorizes no app/runtime/ranking/model change or schedule.
...
The later Sol R8 corrective review in `TRANCHE7CR8_OPERATIONAL_RUNNER_DESIGN.md` narrows that authority into ordered R8A/R8B/R8C targets. R8 is closed. The R7 v1 artifact is preserved but cannot be installed or loaded operationally; the main-only research-capture workflow depends on the separately validated v2 lock and the green combined closure, but does not authorize app or production activation.
