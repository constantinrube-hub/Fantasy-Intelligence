...
Model parameters are canonical JSON only. Ridge uses ordered median/imputer/scaler/coefficient data; HGB uses the repository-owned `fie-hgb-tree-v1` numeric-tree format. No pickle is emitted or read. The validator checks lock and parameter hashes, immutable first-write behavior, governance flags, and portable HGB probe equality against the exported scikit-learn predictions to `1e-10`.
