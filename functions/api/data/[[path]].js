...
export async function onRequestGet(context) {
  ...
  try {
    upstream = await fetch(meta.url, {
      redirect: 'follow',
      headers: { 'User-Agent': 'Fantasy-Intelligence-Engine-V8.9/1.0', Accept: '*/*' }
    });
  } catch (error) {
    return Response.json(
      { error: 'Upstream fetch failed.', source: meta.source, detail: String(error?.message || error) },
      { status: 502, headers: { 'Cache-Control': 'no-store', 'X-FIE-Cache': 'MISS', 'X-FIE-Source': meta.source, 'X-FIE-Fetched-At': new Date().toISOString() } }
    );
  }
  ...
  if (!upstream.ok) {
    ...
    return new Response(body || `Upstream returned ${upstream.status}`, {
      ...
      headers: { 'Content-Type': upstream.headers.get('Content-Type') || 'text/plain; charset=utf-8', 'Cache-Control': 'no-store', 'X-FIE-Cache': 'MISS', 'X-FIE-Source': meta.source, 'X-FIE-Fetched-At': new Date().toISOString() }
    });
  }
