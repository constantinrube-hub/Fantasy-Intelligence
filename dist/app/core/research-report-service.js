...
(function(){
'use strict';
const VERSION='1.0-unified-research-report';
...
async function jsonFetch(path){const r=await fetch(path,{cache:'no-cache'});if(!r.ok)throw new Error(`FIE research fetch ${r.status}: ${path}`);return r.json();}
