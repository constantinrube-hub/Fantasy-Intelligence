...
function computeDemand(ps=players(),league=stateObj()?.league||{},rosters=stateObj()?.rosters||[]){
  const svc=window.FIECore?.LeagueDemandService,pools=poolByPosition(ps);if(!svc?.starterDemand)throw new Error('FIECore.LeagueDemandService unavailable');
  const core=svc.starterDemand({league,players:ps,valueFn:svc.defaultValue});
  const perTeam={};for(const pos of new Set([...POSITIONS,...Object.keys(core.perTeam||{})]))perTeam[pos]=Math.round(Number(core.perTeam?.[pos]||0)*1000)/1000;
  return{teams:core.teams,slots:rosterSlots(league),pools,used:{...(core.used||{})},effectiveDemand:{...(core.leagueStarterDemand||{})},fixedDemand:{...(core.fixedDemand||{})},perTeamDemand:perTeam,slotAllocations:JSON.parse(JSON.stringify(core.slotAllocations||{})),totalStarterSlots:Number(core.starterSlotsPerTeam||0)*Number(core.teams||1),source:'FIECore.LeagueDemandService'};
}
function replacementContext(pos,pool,profile){
  const n=pool.length,d=Math.max(0,Number(profile?.structuralDemand)||0),rank=Math.max(1,Number(profile?.cutoff)||1);if(!n)return{demand:d,starterCutoff:null,replacement:null,replacementRank:null,structuralCutoff:profile?.structuralCutoff??rank,sourceCutoff:profile?.sourceCutoff??rank,source:profile?.source||'FIECore.ReplacementService',scarcityMultiplier:1,depthPressure:0};
  const replacementIdx=Math.min(n-1,Math.max(0,rank-1)),starterIdx=replacementIdx>0?replacementIdx-1:null,starter=starterIdx===null?null:(pool[starterIdx]||null),repl=pool[replacementIdx]||pool[n-1]||null;
  const values=pool.map(playerValue).filter(Number.isFinite).sort((a,b)=>a-b),q25=quantile(values,.25)??0,q75=quantile(values,.75)??100,scale=Math.max(5,q75-q25),gap=starter?Math.max(0,playerValue(starter)-playerValue(repl)):0,depthPressure=Math.min(1,d/Math.max(1,n));
  const scarcity=clamp((d<=0?.8:1)+.55*(gap/scale)+.15*depthPressure,.8,1.5);
  return{demand:d,starterCutoff:starter,replacement:repl,replacementRank:replacementIdx+1,structuralCutoff:profile?.structuralCutoff??replacementIdx+1,sourceCutoff:profile?.sourceCutoff??replacementIdx+1,source:profile?.source||'FIECore.ReplacementService',cutoffConvention:profile?.cutoffConvention||'replacement_player_rank_1_based',scarcityMultiplier:Math.round(scarcity*1000)/1000,depthPressure:Math.round(depthPressure*1000)/1000};
}
function computeEconomics(ps=players(),league=stateObj()?.league||{},rosters=stateObj()?.rosters||[]){
  const demandCtx=computeDemand(ps,league,rosters),byPos=demandCtx.pools,replByPos={},econRows=[],replacementSvc=window.FIECore?.ReplacementService;if(!replacementSvc?.profiles)throw new Error('FIECore.ReplacementService unavailable');
  const coreProfiles=replacementSvc.profiles({league,players:ps,state:stateObj(),valueFn:window.FIECore?.LeagueDemandService?.defaultValue});
  for(const pos of Object.keys(byPos))replByPos[pos]=replacementContext(pos,byPos[pos],coreProfiles[pos]||{position:pos,structuralDemand:0,cutoff:1,source:'FIECore.ReplacementService'});
...
function compute({rerank=true,render=true}={}){
  ...
  try{
    ...
    diagnostics.teams=ctx.teams;diagnostics.totalStarterSlots=ctx.totalStarterSlots;diagnostics.effectiveDemand={...ctx.effectiveDemand};diagnostics.perTeamDemand={...ctx.perTeamDemand};diagnostics.slotAllocations=JSON.parse(JSON.stringify(ctx.slotAllocations||{}));diagnostics.replacement={};for(const [pos,r] of Object.entries(ctx.replacementByPosition||{}))diagnostics.replacement[pos]={demand:r.demand,replacementRank:r.replacementRank,structuralCutoff:r.structuralCutoff,sourceCutoff:r.sourceCutoff,source:r.source,replacement:r.replacement?.name||null,replacementValue:round1(r.replacement?playerValue(r.replacement):null),scarcityMultiplier:r.scarcityMultiplier};
