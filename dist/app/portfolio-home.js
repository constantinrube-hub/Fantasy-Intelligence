...
const Portfolio={version:'1.1.0',mode:true,refreshing:false,snapshots:{},meta:{}};
...
function formatName(e,s){
  const labels={REDRAFT:'Redraft',DYNASTY:'Dynasty',CHOPPED:'Chopped',REDRAFT_BESTBALL:'Redraft + Best Ball',DYNASTY_BESTBALL:'Dynasty + Best Ball'};
  const explicit=String(e?.formatOverride||'').toUpperCase();
  if(explicit&&explicit!=='AUTO')return labels[explicit]||explicit;
  const live=Portfolio.meta[String(e?.id||'')]?.league;
  const resolved=window.FIELeagueProfileResolver?.resolve?.(live,{formatOverride:explicit});
  return labels[resolved?.format]||resolved?.format||s?.format||'Auto-detect';
}
...
function priorityValue(e){return ({VERY_HIGH:4,HIGH:3,MEDIUM:2,LOW:1})[String(e?.priority||'MEDIUM').toUpperCase()]||2;}
function priorityLabel(e){return String(e?.priority||'MEDIUM').replace('_',' ');}
function researchStatus(e){return e?.researchStatus||window.FIEPortfolioConfig?.statusFor?.(e?.id)?.status||'NOT BUILT';}
...
async function sleeperJSON(url){if(typeof window.fetchJSON==='function')return window.fetchJSON(url);return window.FIEDataClient.json(url,{timeoutMs:9000,sourceId:'portfolio-sleeper'});}
async function refreshOne(e){const id=String(e.id);try{const [league,drafts]=await Promise.all([sleeperJSON(`https://api.sleeper.app/v1/league/${id}`),sleeperJSON(`https://api.sleeper.app/v1/league/${id}/drafts`).catch(()=>[])]);const resolved=window.FIELeagueProfileResolver?.resolve?.(league,{formatOverride:e?.formatOverride});Portfolio.meta[id]={league:{league_id:String(league?.league_id||id),name:league?.name||e.name||id,season:league?.season||e.season||'',status:league?.status||'',total_rosters:Number(league?.total_rosters)||null,settings:league?.settings||{},metadata:league?.metadata||{},formatResolved:resolved?.format||null,formatSource:resolved?.source||null},drafts:(drafts||[]).map(d=>({draft_id:String(d.draft_id||''),status:d.status||'',season:d.season||'',type:d.type||''})),checkedAt:new Date().toISOString(),error:null};}catch(err){Portfolio.meta[id]={...(Portfolio.meta[id]||{}),checkedAt:new Date().toISOString(),error:String(err?.message||err)};}}
...
function bind(){
  ...
  // LeagueController is the final authoritative loader. Do not stack another loadLeague wrapper here.
  const lb=document.getElementById('loadBtn');if(lb&&window.FIELeagueController?.switchLeague){lb.onclick=()=>{leavePortfolio();state.activeTab='home';return window.FIELeagueController.switchLeague();};}
